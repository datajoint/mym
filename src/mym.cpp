...
void mexFunction(int nlhs, mxArray*plhs[], int nrhs, const mxArray*prhs[]) {
    ...
    if (nrhs<=jarg)
        ...
        if (!strcasecmp(query, "open"))
            ...
            q = VERSION;
        else if (strcasestr(query, "deserialize") != NULL) // command has 1 input and 1 output variable, converts a matlab blob to variable, deserialize must be tested before serialize when using strcasestr
            q = DESERIALIZE;
        else if (strcasestr(query, "serialize") != NULL) // command has 1 input and 1 output variable, converts a matlab variable to blob 
            q = SERIALIZE;
        else
